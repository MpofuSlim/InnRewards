...
/**
 * Pins the voucher code format: ten digits, first never {@code 0}. The leading
...
class CryptoSignerVoucherCodeTest {
    ...
    @Test
    void everyCodeIsTenDigits_neverStartingWithZero() {
        for (int i = 0; i < SAMPLES; i++) {
            assertThat(CryptoSigner.randomNumericVoucherCode()).matches("[1-9][0-9]{9}");
        }
    }
    ...
    void codesDoNotRepeatInPractice() {
        // 9×10⁹ codes: 20k draws expect ~0.02 collisions, so a meaningful number
        // of repeats means the generator is not drawing from the full space.
